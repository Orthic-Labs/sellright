...
- [x] **DEP-1 — production dependency audit clean.** `pnpm deps:audit` passed again in evidence-head CI run `34031428018` on evidence head `7e3ed35`, after the admin React Router advisory fix (`bb54899`) and storefront transitive security floors (`4eb150d`).
...
- [x] **CI-1 — include dependency audit in CI.** Landed in `e9b1b67`; evidence-head CI run `34031428018` proves the production dependency audit executes after frozen installs and passes.
...
- [x] **OPS-2 — backup/restore drill.** Production-container CI proves backup and restore into a disposable database; evidence-head run `34031428018` passed the `Prove database backup and restore` step.
- [x] **OPS-3 — installation/reboot smoke.** Production-container evidence-head CI run `34031428018` passed first-run/startup checks plus `Prove persistent-volume reboot and idempotent bootstrap`.
