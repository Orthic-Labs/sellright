  ...
  async (c) => guard(c, async () => {
    ...
    const out = await withStore(st.storeId, async (tx) => {
      ...
      const [p] = await tx.insert(s.blogPost).values({
        storeId: st.storeId, title: b.title, slug, excerpt: b.excerpt ?? null, body: b.body ?? null, bodyHtml: b.body ?? null,
...
adminContent.openapi(
  ...
  async (c) => guard(c, async () => {
    ...
    const ok = await withStore(st.storeId, async (tx) => {
      ...
      if (b.body !== undefined) { patch.body = b.body; patch.bodyHtml = b.body; patch.readingTime = readingTime(b.body ?? ''); }
