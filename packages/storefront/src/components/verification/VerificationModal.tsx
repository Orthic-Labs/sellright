            ...
            <div class="grid grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-4">
              {VERIFICATION_PROGRAMS.map((program) => (
                <button
                  ...
                  class="group relative p-3 sm:p-4 lg:p-6 bg-white border-2 border-gray-200 rounded-xl hover:border-[var(--color-accent)] hover:shadow-lg transition-all duration-300 text-left min-h-[140px] sm:min-h-[160px] cursor-pointer"
                >
                  ...
                  <div class="relative flex flex-col h-full">
                    {/* Program name */}
                    <h3 class="text-sm sm:text-base lg:text-lg font-bold text-gray-900 mb-2 sm:mb-3 group-hover:text-[var(--color-accent)] transition-colors leading-tight">
                      {program.name}
                    </h3>
                    ...
                    <div class="flex items-center justify-between">
                      {/* Discount badge */}
                      <div class="inline-flex items-center bg-[var(--color-accent)] text-white text-xs sm:text-sm font-bold px-2 sm:px-3 py-1 rounded-full">
                        {program.discountPercent}% OFF
                      </div>

                      {/* Arrow icon */}
                      <svg class="w-4 h-4 sm:w-5 sm:h-5 text-gray-400 group-hover:text-[var(--color-accent)] transform group-hover:translate-x-1 transition-all duration-300" fill="none" stroke="currentColor" viewBox="0 0 24 24">
