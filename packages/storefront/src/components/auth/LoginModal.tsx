...
import { clearCustomerCacheAfterMutation } from '~/services/customer';
...
export default component$<LoginModalProps>(({
  ...
  const turnstileToken = useSignal(''), honeypot = useSignal('');
  ...
  useVisibleTask$(({ track, cleanup }) => {
    ...
    cleanup(() => {
      clearTimeout(timer);
      if (widgetId.value) (window as any).turnstile?.remove(widgetId.value);
      ...
      <div class="relative w-full max-w-md mx-4">
        <div class="bg-[#FDFAF6] rounded-[3px] border border-[#D8D1C7] shadow-none overflow-hidden">
          <div class="px-6 py-6">
            ...
            <div id="modal-turnstile" class={['email', 'signin', 'signup'].includes(step.value) ? 'min-h-[65px] flex justify-center mb-4' : 'hidden'}></div>
            {step.value === 'email' && (
              <div class="space-y-4 font-body">
                ...
                <button
                  key={`email-continue-${loading.value}`} onClick$={handleEmailContinue} disabled={loading.value}
            ...
            {step.value === 'signin' && (
              <div class="space-y-4 font-body">
                ...
                <div class="flex items-center justify-between">
                  ...
                  <button key={`password-reset-${loading.value}`} onClick$={handleForgotPassword} disabled={loading.value}
                ...
                <button key={`signin-submit-${loading.value}`} onClick$={handleSignIn} disabled={loading.value}
                  class="w-full flex justify-center py-3 px-4 border border-transparent rounded-[3px] text-sm font-medium text-[#FDFAF6] bg-[#141210] hover:opacity-90 transition-colors cursor-pointer disabled:opacity-50">
            ...
            {step.value === 'signup' && (
              <div class="space-y-4 font-body">
                ...
                <button key={`signup-submit-${loading.value}`} onClick$={handleSignUp} disabled={loading.value}
                  class="w-full flex justify-center py-3 px-4 border border-transparent rounded-[3px] text-sm font-medium text-[#FDFAF6] bg-[#141210] hover:opacity-90 transition-colors cursor-pointer disabled:opacity-50">
            ...
            {step.value === 'success' && (
              <div class="text-center py-4">
                <div class="bg-[var(--color-parchment)] border border-[#D8D1C7] rounded-[3px] p-6">
                  <svg class="w-12 h-12 text-[var(--color-accent)] mx-auto mb-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7" />
                  </svg>
                  <h3 class="text-lg font-semibold text-[var(--color-ink)] mb-2">Check your email</h3>
                  <p class="text-gray-600 mb-4">
                    We sent a verification link to <span class="font-medium">{email.value}</span>. Click the link to activate your account.
                  </p>
                  <button onClick$={handleBack}
                    class="text-sm font-medium text-[var(--color-accent)] hover:text-[var(--color-ink)] cursor-pointer">
                    Back to sign in →
                  </button>
                </div>
              </div>
