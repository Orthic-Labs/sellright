...
import { effectiveUnitPriceCents } from './effective-price';
import { srAssetUrl } from './sellright';
...
/**
 * GET /catalog/search → SearchResponse-ish. Pricing metadata is from the
 * enabled variant supplying the product's minimum effective price.
 * Stock comes from enabled, purchasable variants, never product publication status.
 */
export function adaptSearch(res: SrSearchResult): AdaptedSearchResponse {
  return {
    ...
    items: res.items.map((p) => ({
      ...
      productAsset: p.image ? { id: p.slug, preview: srAssetUrl(p.image) } : null,
    ...
    facetValues: [],
    collections: [],
    itemCustomFields: res.items.flatMap(p => p.pricingVariant ? [{ productVariantId: p.pricingVariant.sku, salePrice: p.pricingVariant.salePrice, preOrderPrice: p.pricingVariant.preOrderPrice, isPreOrder: p.pricingVariant.isPreOrder, shipDate: p.pricingVariant.shipDate }] : []),
  };
}
...
/**
 * GET /catalog/products/{slug} → Product-ish, retaining option identities so
 * the live fallback has the same selector behavior as a generated snapshot.
 */
export function adaptProduct(res: SrProductDetail): AdaptedProduct {
  const featuredAsset = res.images[0] ? { id: `${res.slug}-0`, preview: srAssetUrl(res.images[0]) } : null;
  const assets = res.images.map((preview, i) => ({ id: `${res.slug}-${i}`, preview: srAssetUrl(preview) }));
  return {
    id: res.slug,
    name: res.name,
    slug: res.slug,
    ...
    variants: res.variants.map((v) => ({
      ...
      sku: v.sku,
      price: v.price,
      priceWithTax: effectiveUnitPriceCents(v.price, v),
    ...
    facetValues: (res.tags ?? []).map(name => ({ id: name, name, code: name, facet: { id: 'tags', name: 'Tags', code: 'tags' } })),
    customFields: {},
...
/** A collection-grid product, shaped for ProductCard (same fields adaptSearch
 *  produces per item — image/inStock/salePrice/preOrderPrice computed live on
 *  the backend, no cache) so the collection page can render full tiles
 *  instead of a bare text tile. */
export interface AdaptedCollectionProduct {
  slug: string;
  name: string;
...
export interface AdaptedCollection {
  ...
  products: AdaptedCollectionProduct[];
  total: number;
  page: number;
  pageSize: number;
}

export function adaptCollection(res: SrCollection & Partial<{ total: number; page: number; pageSize: number }>): AdaptedCollection {
  return {
    id: res.slug,
    slug: res.slug,
    name: res.name,
    ...
    breadcrumbs: [],
    parent: { name: '__root_collection__' },
    productVariants: { items: [], totalItems: res.total ?? res.products.length },
    ...
    page: res.page ?? 1,
    pageSize: res.pageSize ?? res.products.length,
  };
}
