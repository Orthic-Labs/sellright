...
export default component$(() => {
    useStyles$(`
        :root .aff-scope { --aff-accent: var(--color-accent); --aff-ink: var(--color-ink); --aff-muted: var(--color-ink-soft); --aff-bg: var(--color-parchment); --aff-surface: #ffffff; --aff-border: var(--color-card-border); --aff-soft: var(--color-parchment-hover); }
        ...
        .aff-kicker { font-family: var(--font-mono); font-size: 11px; letter-spacing: 0.1em; text-transform: uppercase; color: var(--aff-muted); margin-bottom: 18px; display: flex; align-items: center; gap: 10px; }
        .aff-kicker::before { content: ''; display: inline-block; width: 28px; height: 1px; background: var(--aff-accent); }

        .aff-h1 { font-family: var(--font-display); font-size: clamp(2rem, 5vw, 3rem); line-height: 1.05; font-weight: 600; margin: 0 0 0.5rem; letter-spacing: -0.01em; }
        ...
        .aff-pill { font-family: var(--font-mono); font-size: 12px; letter-spacing: 0.06em; padding: 4px 10px; background: var(--aff-ink); color: #fff; border-radius: 2px; }
        ...
        .aff-card-hero { background: var(--aff-surface); border: 1px solid var(--aff-accent); border-radius: 8px; padding: 1.5rem 1.75rem; box-shadow: 0 1px 0 rgba(var(--color-accent-rgb),0.08); }
        .aff-card .label, .aff-card-hero .label { font-family: var(--font-mono); font-size: 10.5px; letter-spacing: 0.1em; text-transform: uppercase; color: var(--aff-muted); margin-bottom: 12px; }
        .aff-card .value { font-family: var(--font-display); font-size: clamp(2rem, 4.5vw, 2.625rem); line-height: 1; font-weight: 600; font-variant-numeric: tabular-nums; letter-spacing: -0.01em; color: var(--aff-ink); }
        .aff-card-hero .value { font-family: var(--font-display); font-size: clamp(2.25rem, 5vw, 3rem); line-height: 1; font-weight: 600; font-variant-numeric: tabular-nums; letter-spacing: -0.01em; color: var(--aff-accent); }

        .aff-section-h { font-family: var(--font-display); font-size: 1.625rem; font-weight: 600; margin: 3rem 0 0.875rem; letter-spacing: -0.005em; }
        ...
        .aff-table th { padding: 14px 18px; text-align: left; border-bottom: 1px solid var(--aff-border); background: var(--aff-soft); font-family: var(--font-mono); font-size: 10.5px; letter-spacing: 0.08em; text-transform: uppercase; color: var(--aff-muted); font-weight: 500; white-space: nowrap; }
        ...
        .aff-table td.num { text-align: right; font-family: var(--font-mono); font-variant-numeric: tabular-nums; white-space: nowrap; }
        .aff-table td.mono { font-family: var(--font-mono); font-size: 13px; }
        .aff-table td.muted { color: var(--aff-muted); font-size: 13px; }
        .aff-table td.commission { color: var(--aff-accent); }
        ...
        .aff-footnote a { color: var(--aff-accent); text-decoration: none; }
        .aff-footnote a:hover { text-decoration: underline; }

        .aff-empty { text-align: center; padding: 5rem 1.5rem; }
        .aff-empty-h { font-family: var(--font-display); font-size: 1.875rem; font-weight: 500; margin-bottom: 12px; color: var(--aff-ink); }
        ...
        <div class="aff-scope">
            <div class="aff-page">
                ...
                <h2 class="aff-section-h">
                    Orders{' '}
                    <span style="font-family: var(--font-mono); font-size: 13px; color: var(--aff-muted); letter-spacing: 0.04em; font-weight: 400;">
                        ({totals.orderCount})
                    </span>
                </h2>
                {orders.length === 0 ? (
                ...
                <p class="aff-footnote">
                    Refunded or cancelled orders are not shown. Commission is 10% of the post-discount, pre-tax product subtotal — shipping and tax are excluded. Payouts go out monthly. Lost your link? <a href={`mailto:${theme.supportEmail}`}>Email us</a> and we'll resend it.
                </p>
            </div>
        </div>
    );
...
export const head: DocumentHead = createSEOHead({
    title: `Affiliate dashboard — ${theme.storeName}`,
    description: `${theme.storeName} affiliate stats.`,
    noindex: true,
});
