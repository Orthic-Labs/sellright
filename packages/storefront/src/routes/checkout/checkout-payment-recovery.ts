...
interface RecoverCheckoutPaymentErrorOptions {
  ...
  /** SR path: the PendingPayment order the failed attempt was against — its
   *  truth lives on SellRight, not the (empty) Vendure active order. */
  srOrder?: { code: string; receiptToken?: string };
}
...
export async function recoverCheckoutPaymentError(options: RecoverCheckoutPaymentErrorOptions): Promise<void> {
  ...
  try {
    // SR path: a failed client call does not mean the charge failed — the
    // gateway may have settled (e.g. response lost on a slow network). Read
    // the actual order before telling the shopper to retry.
    if (SR_CHECKOUT_ENABLED && srOrder?.code) {
      try {
        const sr = await srGetOrder(srOrder.code, srOrder.receiptToken);
        if (sr.state === 'Paid') {
          paymentComplete.value = true;
          const rt = srOrder.receiptToken ? `?rt=${encodeURIComponent(srOrder.receiptToken)}` : '';
          navigate(`/checkout/confirmation/${sr.code}${rt}`);
          return;
        }
      } catch (e) {
        console.error('[Checkout] SR order state check failed:', e);
      }
      // Order still payable — reset and let the shopper retry the payment.
