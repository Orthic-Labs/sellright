...
const ConfirmationPage = component$(() => {
	...
	useVisibleTask$(async () => {
		try {
			...
			// Sezzle return: the hosted complete_url carries ?paymentAttempt={id}
			// (server-set). Reconcile the attempt with the provider BEFORE reading
			// the order — verify settles the payment server-side so the order read
			// below already reflects the outcome. Deduped per session so refresh
			// doesn't re-verify (the server-side call is idempotent regardless).
			const paymentAttempt = loc.url.searchParams.get('paymentAttempt');
			if (paymentAttempt) {
				const verifyKey = `gw-verified-${code}-${paymentAttempt}`;
				if (!sessionStorage.getItem(verifyKey)) {
					sessionStorage.setItem(verifyKey, 'true');
					try {
						const attempt = await srVerifyGatewayPayment(code, paymentAttempt, { receiptToken: rt });
						if (attempt.status === 'declined' || attempt.status === 'failed') {
							store.sezzleVerificationFailed = true;
							store.sezzleErrorMessage = 'Payment was not completed. No charge was made — you can try again.';
							store.loading = false;
							return;
						}
					} catch {
			...
			// Tolerate webhook lag: gateways and Stripe redirect here the instant
			// the shopper returns, but provider settlement (→ Paid) may land a
			...
			if (store.order?.id) {
				...
				if (hasSuccessfulPayment) {
					...
					if (SERVER_CART_ENABLED) {
						// The cart converted server-side — it's terminal, so a PATCH to
						// clear it would 409. Retire the local mirror + token only.
		...
		<div class="bg-[var(--color-parchment)] min-h-screen">
			...
			{store.sezzleVerificationFailed && (
				<div class="max-w-2xl mx-auto pt-20 pb-24 px-6 text-center">
					...
					<h1 class="font-display text-4xl text-[var(--color-ink)] mb-3" style="line-height: 1.1">Payment verification error</h1>
					<p class="text-[#5b5a56] text-sm mb-4">{store.sezzleErrorMessage}</p>
					<p class="text-xs text-[#5b5a56] mb-8">
						If payment has been deducted, please contact support with order code <span class="font-medium text-[var(--color-ink)]">#{code}</span>
					</p>
					<div class="flex flex-wrap gap-6 justify-center text-xs tracking-[0.14em] uppercase">
						<button
							onClick$={() => {
								...
								if (attempt) sessionStorage.removeItem(`gw-verified-${code}-${attempt}`);
								window.location.reload();
							}}
							class="text-[var(--color-ink)] border-b border-[var(--color-ink)] pb-0.5 hover:opacity-70 transition-opacity"
						>
							Retry verification
						</button>
				...
				<div class="max-w-3xl mx-auto pt-8 sm:pt-12 pb-24 px-6">
					...
					<div class="text-center mb-12 sm:mb-14">
						...
						<p class="text-[#5b5a56] text-sm mb-1">
							Your order <span class="font-medium text-[var(--color-ink)]">#{store.order.code}</span> is confirmed
						</p>
					...
					<div class="border-t border-[#E5E0D8] pt-8 mb-10">
						...
						<div class="border-t border-[#E5E0D8] pt-4 mt-4 space-y-2 max-w-xs ml-auto">
							...
							{(store.order.discounts?.length ?? 0) > 0 && store.order.discounts?.map((discount, idx) => (
								<div key={idx} class="flex justify-between text-xs text-[var(--color-accent)]">
							...
							<div class="flex justify-between pt-3 border-t border-[#E5E0D8]">
								...
								<span class="text-sm font-medium text-[var(--color-ink)] tabular-nums">{formatPrice(store.order.totalWithTax || 0)}</span>
							</div>
						</div>
					</div>
					...
					<div class="border-t border-[#E5E0D8] pt-8 grid grid-cols-1 sm:grid-cols-2 gap-8 mb-12">
						...
						{hasBilling && billAddr && (
							<div>
								...
								<address class="not-italic text-xs text-[#5b5a56] leading-relaxed">
									{billAddr.fullName && <div class="text-sm text-[var(--color-ink)]">{billAddr.fullName}</div>}
						...
						{store.order.shippingLines?.length ? (
							<div>
								...
								{store.order.shippingLines.map((line, idx) => (
									<p key={idx} class="text-sm text-[var(--color-ink)]">{line.shippingMethod.name}</p>
								))}
							</div>
						) : null}
